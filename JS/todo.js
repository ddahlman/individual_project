(function () {
    ...
    document.querySelector('#submit').addEventListener('click', function (e) {
        ...
        if (obj.headline === '' || obj.todoItem[0] === '') {
            ...
            Array.from(document.querySelectorAll('#headline, #todoItem, #bestBefore')).map(function (inp) { inp.value = ''; return inp.value; });
            Array.from(document.querySelectorAll('#head-error, #todo-error')).map(function (inp) { inp.value = ''; return inp.value; });
            show();
        }
    });
    function removeTodo() {
    ...
    function addTask() {
        ...
        Array.from(document.querySelectorAll('.save-item')).map(function (btn) {
            btn.addEventListener('click', function () {
                var input = this.parentNode.parentNode.querySelector('.this-item').value;
                var test = todos.map(function (obj, i) {
                    if (id === 'nr' + i + '') {
    ...
    function show() {
        let todos = getTodos();
        let todoArray = todos.map(function (item, i) {
            ...
            if (now < bf3Days) {
                ...
                labelborder = '<h5 class="expired">Försenad!</h5>';
            }
            else if (now <= itemBf && now >= bf3Days) {
                panelclass = 'panel panel-warning';
                labelborder = '<h5 class="warning">Påminnelse!</h5>';
            }
            else {
                panelclass = 'panel panel-info';
                labelborder = '';
            }
            let todoitem = item.todoItem.map(function (li, i) {
            ...
            let todo = '<div id="div' + i + '" class="' + panelclass + ' todo">' +
                '<div class="panel-heading clearfix">' + labelborder + '<strong>' + item.headline + '</strong><br/>' +
                '<span class="item-today">utfärdades: ' + item.today + '</span><br/>' +
                '<strong>gör innan: </strong>' + item.bestBefore + '' +
                '<div class="btn-group pull-right">' +
        ...
        Array.from(document.querySelectorAll('.remove')).map(function (btn) { btn.addEventListener('click', removeTodo); return btn; });
        Array.from(document.querySelectorAll('.addTask')).map(function (btn) { btn.addEventListener('click', addTask); return btn; });
        Array.from(document.querySelectorAll('.rmv')).map(function (btn) { btn.addEventListener('click', removeItem); return btn; });
    }
