...
#
class _cv_education extends Resource{ // Klassen ärver egenskaper från den generella klassen Resource som finns i resource.class.php
    # Här deklareras de variabler/members som objektet ska ha
    public  $id, $year, $education, $educations, $request;
    ...
    private function getUserData($input, $connection){
        if($this->id){ // Om vår URL innehåller ett ID på resursen hämtas bara den usern
            ...
            $this->education = $education['school'];
    ...
    function POST($input, $connection){
        ...
        $education = escape($input['education']);
        $year = escape($input['year']);
        
        $query = "INSERT INTO education (school, year)
    ...
    function PUT($input, $connection){
        ...
        if($this->id){
            ...
            SET school = '$education', year = '$year'
            WHERE id = $this->id
            ";
