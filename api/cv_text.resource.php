...
#
class _cv_text extends Resource{ // Klassen ärver egenskaper från den generella klassen Resource som finns i resource.class.php
    # Här deklareras de variabler/members som objektet ska ha
    public  $id, $text, $texts, $request;
    ...
    function PUT($input, $connection){
        ...
        if($this->id){
            $text = escape($input['first_text']);
            
            $query = "UPDATE cv_firsttext
            SET first_text = '$text'
            WHERE id = $this->id";
